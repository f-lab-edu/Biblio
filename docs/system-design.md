...
1. 프로젝트 기반 업로드 및 검색 UI 제공: 사용자는 프로젝트를 만들고 그 안에 로컬 파일 또는 외부 URL 영상을 추가한다. 검색도 선택한 프로젝트 범위에서 수행하며, 결과(근거 구간/타임스탬프)를 표시한다. 타임스탬프 클릭 시 영상 재생은 영상 출처(input_type)에 따라 분기 처리한다. LOCAL_FILE은 Core API Server로부터 재생용 보안 URL을 동적으로 발급받아 내장 임베디드 플레이어로 재생하고, EXTERNAL_URL은 백엔드를 거치지 않고 원본 외부 링크(source_url)와 타임스탬프 정보를 활용하여 클라이언트 내부 계층에서 직접 외부 플랫폼과 연동하여 재생 위치를 제어한다.
2. 처리상태 조회 및 목록 관리: 프로젝트 목록과 프로젝트 내 영상 목록을 조회하고, 제목 수정/삭제, 처리 상태(업로드 완료/처리 중/완료)를 확인한다.
...
1. 사용자 요청 처리: 프로젝트 생성/조회, 프로젝트 내 영상 업로드, 처리 상태 조회, 영상 목록 관리 등 클라이언트 요청을 처리한다.
2. presigned URL 발급: 프로젝트 소유권 확인 후 업로드 가능한 URL과 영상 고유 id 를 발급
3. 메타데이터 및 초기 상태 저장: 프로젝트와 영상에 대한 메타데이터(업로더, 영상이름, 카테고리)와 초기 상태를 db에 저장
4. 업로드 완료 확인/처리 트리거: 클라이언트로 부터 업로드가 끝났다는 신호를 받아 파이프라인 작업을 큐에 넣고, 상태를 업데이트
5. 인증 및 인가(AuthN/AuthZ): 전달받은 Access Token(JWT)의 서명과 만료를 내부 미들웨어에서 직접 검증한다. 이후 claim에서 추출한 requester_user_id를 기준으로 프로젝트 소유권을 확인하여, 본인이 소유한 프로젝트와 그 하위 영상 데이터에만 접근하도록 제어한다.
...
8. 영상 삭제 요청 처리: 사용자의 삭제 요청을 수신하여 프로젝트 소유권을 확인하고 Video.status를 DELETING으로 전이한 뒤 DELETE_REQUEST를 메시지 브로커에 발행한다. 실제 연쇄 삭제는 Pipeline Worker에 위임한다.
...
10. 전체 파이프라인 상태 조회: 소유권 제한 없이 프로젝트와 임의 video_id의 처리 현황 및 실패 상세를 조회한다.
...
검색 스코프는 인증된 사용자와 그 사용자가 소유한 단일 프로젝트의 교집합이다. Search Service는 프로젝트 소유권을 먼저 검증하고, Metadata DB 및 Vector Store 조회와 최종 SOT 게이트에서 같은 스코프를 강제한다. 롤백 복구 중인 프로젝트는 해당 프로젝트만 검색에서 일시 제외된다.

1. 검색 전담 처리: 자연어 질의를 수신하며, 정의된 검색 스코프 안의 프로젝트가 검색 준비 상태일 때 하이브리드 검색 파이프라인(임베딩 변환 및 DB/Vector 조회)을 시작한다.
2. 하이브리드 검색 오케스트레이션 (Search Orchestrator):
   ...
   - 최종 응답 전에 Metadata DB(SOT)에서 최종 서빙 검증을 수행하여 권한, 존재 여부, 프로젝트 범위, 검색 가능 상태를 통과한 Chunk만 반환한다.
3. LLM 컨텍스트 주입: 추출된 청크들을 조립한 프롬프트를 Search Service 내부 LLM 연동 구현체에 전달하여 최종 답변을 생성하며, 피드백 귀속용 `req_id`, 타임스탬프 레퍼런스 `ref`, 실제 인용 여부 `used`가 포함된 `chunks` 배열을 함께 반환한다. 프로젝트가 롤백 복구 중이라 검색에서 제외된 경우에는 그 사실을 사용자에게 함께 고지한다.
4. 인증 및 테넌시 강제(AuthN/AuthZ): 전달받은 Access Token(JWT)을 직접 검증하여 requester_user_id를 추출하고, 검색 스코프와 테넌시 원칙을 모든 조회 단계에 적용한다.
5. 검색 응답 스냅샷 저장: 검색 응답 반환 시, 피드백 검증과 운영 추적에 필요한 응답 스냅샷(프로젝트, 질의, 최종 청크, active 모델 정보, 실제 조회한 벡터 검색 경로 정보, 검색 가능 범위 상태)을 Metadata DB에 단기 보존(TTL)한다.
...
3. 메시지 계약(Message Contract): 파이프라인 메시지는 공통 Envelope + 메시지별 Payload로 구성하며, 최소 필드/스키마는 Data Model의 “Message Contract(3.13)” 정의를 따른다.
...
5. 검색 색인 구축 및 완료: 전체 대본/청크 메타데이터는 Metadata DB(SOT)에 트랜잭션 적재하고, 임베딩 벡터는 Vector Store(ANN)에 Upsert 한다. 반영 완료 시 Video.status를 READY로 갱신한다. 프로젝트 검색 가능 여부는 프로젝트 상태와 프로젝트 내 영상 처리 상태를 함께 따른다.
...
8. 복구 후 후처리: rollback 이후 문제 모델 기간에 생성된 기존 데이터가 포함된 프로젝트는 복구가 끝날 때까지 검색 범위에서 제외한다. 해당 프로젝트의 데이터는 복원된 정상 서빙 상태 기준으로 백그라운드 재임베딩하며, 복구가 끝난 프로젝트부터 다시 검색 범위에 합류시킨다. 이 재임베딩은 rollback 완료와 분리된 후속 복구 작업이다.
...
1. 정합성 보장(SOT): 사용자, 프로젝트, 영상 메타데이터, 상태(Status), Transcript/Chunk(텍스트/타임스탬프/참조)를 ACID 트랜잭션으로 저장한다.
...
1. 벡터 저장(파생 인덱스): Chunk 단위 임베딩 벡터를 저장하며, 검색 스코프 필터를 적용할 수 있도록 최소 메타데이터(user_id, project_id, video_id 등)를 함께 보유한다.
...
4. **프로젝트 검색 노출 상태:** `READY`는 영상 단위 파이프라인 기본 처리 완료를 뜻한다. 평상시 프로젝트 검색은 프로젝트 내 모든 영상이 준비된 경우에만 허용된다. rollback 복구 중인 프로젝트는 별도 검색 노출 상태에 따라 일시적으로 검색 범위에서 제외된다.
...
- 검색 범위 제한: 문제 모델 기간에 생성된 기존 데이터가 포함된 프로젝트는 복구가 끝날 때까지 검색 범위에서 제외한다. Search Service는 다른 검색 가능한 프로젝트의 검색은 계속 제공하고, 제외된 프로젝트에는 복구 중임을 사용자에게 고지한다.
- 복구 후 후처리: 문제 모델 기간에 생성된 기존 데이터는 복원된 정상 서빙 상태 기준으로 재임베딩하며, 복구가 끝난 프로젝트부터 다시 검색 범위에 합류시킨다. 이 작업은 rollback 완료와 분리된 후속 복구 작업으로 진행한다.
...
**입력**
- 주체: Client
- 데이터: 프로젝트 식별자, 로컬 영상 파일 (binary) 또는 외부 영상 URL, 메타데이터 (제목, 카테고리)

**처리**
1. Core API Server가 요청자가 소유한 프로젝트를 확인한 뒤 고유 식별자(UUID 등)를 직접 생성하여 video_id를 할당하고, Object Storage에 저장될 객체 키(storage_path)를 미리 결정한다.
2. Core API Server가 프로젝트 연결 정보, 메타데이터(제목, 카테고리, 원본 URL 등)와 확정된 storage_path를 한 번의 트랜잭션으로 Metadata DB에 저장한다. (status=PENDING)
...
| 영상 메타데이터 (프로젝트, 제목, 카테고리, 업로더, source_url, status=PENDING/UPLOADED) | Metadata DB |
...
**입력**
- 주체: Client
- 데이터: 프로젝트 식별자, 자연어 질의 텍스트, Authorization 토큰

**처리**
1. Reverse Proxy가 요청을 수신하여 Authorization 헤더를 포함한 원본 요청을 그대로 Search Service로 전달
2. Search Service가 내부 미들웨어를 통해 JWT를 직접 검증하고, claim에서 requester_user_id를 추출하여 프로젝트 소유권을 확인한다.
3. Search Service가 프로젝트 검색 노출 상태와 프로젝트 내부 영상 처리 상태를 확인한다. 프로젝트가 롤백 복구 중이거나 프로젝트 내 영상이 모두 준비되지 않았으면 검색 파이프라인을 시작하지 않는다.
4. 질의 텍스트를 **Managed Embedding Endpoint**로 직접 보내 active 모델 기준 임베딩 벡터로 변환한다. previous 세대가 병행 서빙 중이면 동일 질의를 previous 모델 기준으로도 추가 임베딩한다.
5. Search Service가 Metadata DB(FTS)에서 키워드 후보 Top-K를 조회한다. 조회 범위는 검색 스코프와 테넌시 원칙을 따른다.
6. Search Service가 Vector Store(ANN)에서 active 인덱스의 벡터 후보 Top-K를 조회한다. previous 세대가 병행 서빙 중이면 previous 인덱스도 별도로 조회한다. 조회 범위는 검색 스코프와 테넌시 원칙을 따른다.
7. Search Service가 키워드 후보와 active/previous 벡터 후보를 병합(RRF)하여 최종 Top-K 후보를 결정한다.
8. Search Service가 Metadata DB(SOT) 를 “서빙 게이트”로 조회하여 권한, 존재 여부, 프로젝트 범위, 검색 가능 상태 검증을 수행하고, 최종 컨텍스트(청크 텍스트/타임스탬프)를 로드한다. 이 단계 이후에도 최종 컨텍스트가 0개면 LLM을 호출하지 않고 `"검색 결과가 없습니다"`를 반환한다.
9. Search Service가 Top-K 컨텍스트 + 질의를 서비스 내부 LLM 인터페이스 구현체에 전달하여 최종 답변과 structured `used_refs`를 생성한다.
10. Search Service가 검색 응답을 Client에 반환한다. 동시에 피드백 검증과 운영 추적에 사용할 수 있도록, 요청 시점의 프로젝트, 응답 내용, active 모델 정보, 실제 조회한 벡터 검색 경로 정보, 검색 가능 범위 상태를 포함한 검색 응답 스냅샷을 불변 기록으로 저장한다.
...
**처리**
1. Core API Server가 JWT를 검증하여 requester_user_id를 추출하고 프로젝트 소유권을 확인한다.
...
8. 문제 모델 기간에 생성된 기존 데이터가 포함된 프로젝트는 복구가 끝날 때까지 검색 범위에서 제외한다. Search Service는 다른 검색 가능한 프로젝트의 검색은 계속 제공하고, 제외된 프로젝트에는 복구 중임을 사용자에게 고지한다. 해당 데이터는 복원된 정상 서빙 상태 기준으로 재임베딩하며, 복구가 끝난 프로젝트부터 다시 검색 범위에 합류시킨다.
...
## 3.1 Project
사용자가 영상을 묶어 검색하는 논리적 작업 단위
- id: 프로젝트 고유 ID
- user_id: 프로젝트 소유 사용자 ID
- title: 프로젝트 제목
- description: 프로젝트 설명 (선택)
- search_serving_state: 프로젝트 검색 노출 상태 (`SERVABLE` / `ROLLBACK_EXCLUDED`). 기본값은 `SERVABLE`이며, rollback 복구 중인 프로젝트는 일시적으로 `ROLLBACK_EXCLUDED`가 된다.
- created_at: 프로젝트 생성 시각
- updated_at: 프로젝트 변경 시각

## 3.2 Video
영상 업로드 시 생성되는 기본 메타데이터 단위
- id: 영상 고유 ID
- project_id: 소속 프로젝트 ID (Project 참조)
- user_id: 업로드 요청자 ID. 접근 제어의 기준은 Project.user_id이며, 이 값은 감사와 운영 추적에 사용한다.
- title: 영상 제목
...
- project_id: 피드백 대상 검색 응답이 생성된 프로젝트 ID
...
- project_serving_state: 검색 시점의 프로젝트 검색 노출 상태
- scope_notice: 프로젝트 검색 가능 상태를 사용자에게 고지한 문구 또는 구조화된 코드
...
## 3.9 ModelEvaluationDetail Artifact
...
## 3.10 VectorIndexEntry (Derived; Vector Store)
- index_name: 모델 버전별 물리 분리 인덱스 식별자. 서빙 대상 인덱스는 릴리스 레코드의 active/previous 조합 기준으로 결정된다.
- chunk_id: Chunk 고유 ID (SOT의 Chunk.id와 동일 키)
- user_id: 테넌시 필터 및 운영 추적용
- project_id: 사용자가 소유한 프로젝트 검색 범위 필터용
- video_id: 영상 추적 및 최종 SOT 검증용
