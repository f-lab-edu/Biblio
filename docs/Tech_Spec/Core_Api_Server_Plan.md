...
- 병렬 작업은 파일 수정 범위가 겹치지 않도록 `src/core|middlewares|schemas|api`, `alembic|models|infra/db`, `infra/storage|infra/broker` 축으로 먼저 분리한다.
- `src/services/video_service.py`와 `src/api/v1/routers/videos.py`는 최종 통합 지점이므로, 선행 작업이 모두 닫힌 뒤 통합 담당자가 합치는 것을 원칙으로 한다.
...
- **Merge Owner / Integration Point:** 최종 통합은 `src/services/video_service.py`, `src/api/v1/routers/videos.py`, `tests/api/v1/test_videos.py`에서 수행한다. 이 지점에서 Shared Contract 검토와 E2E 테스트를 함께 닫는다.
...
- [ ] **Task 0: 앱 엔트리포인트 및 설정 스캐폴딩**
  - **Output:** FastAPI 앱 팩토리, `Settings` 클래스, 라우터 마운트, 기본 DI 진입점, `.env.example`
  - **Files:** `src/main.py`, `src/core/config.py`, `src/core/dependencies.py`, `.env.example`
  - **Test Files:** `tests/unit/test_config.py`
  - **Commands:** `pytest tests/unit/test_config.py`
  - **Verify:** 필수 환경 변수 누락 시 기동이 실패하고, 정상 설정에서는 앱이 기동한다.
  - **Linked AC:** SPEC §1.2, SPEC §5.1 공통 전제
  - **Depends On:** 없음
  - **병렬 가능:** N

- [ ] **Task 1: 인증/에러 계약/DTO/라우터 스켈레톤**
  - **Output:** JWT 검증, `requester_user_id` 추출, 공통 에러 바디(`code`, `message`, `trace_id`), video 요청/응답 DTO, video 라우터 스켈레톤
  - **Files:** `src/middlewares/auth.py`, `src/middlewares/error_handler.py`, `src/schemas/video_dto.py`, `src/api/v1/routers/videos.py`
  - **Test Files:** `tests/unit/test_auth_middleware.py`, `tests/unit/test_video_dto.py`
  - **Commands:** `pytest tests/unit/test_auth_middleware.py tests/unit/test_video_dto.py`
  - **Verify:** JWT 미제공/만료/권한 위반/잘못된 payload가 spec의 에러 계약대로 매핑된다.
  - **Linked AC:** SPEC §2.1, SPEC §2.4, SPEC §5.1 인증 및 입력 검증 시나리오
  - **Depends On:** Task 0
  - **병렬 가능:** Y

- [ ] **Task 2: Video 모델, Alembic, Repository, Cursor 구현**
  - **Output:** `Video` ORM 모델, Alembic 마이그레이션, keyset pagination용 cursor 유틸, `VideoRepository`
  - **Files:** `alembic/env.py`, `alembic/versions/0001_init_video.py`, `src/models/video.py`, `src/infra/db/video_repository.py`, `src/infra/db/cursor.py`
  - **Test Files:** `tests/integration/test_video_repository.py`
  - **Commands:** `alembic upgrade head`, `pytest tests/integration/test_video_repository.py`
...
- [ ] **Task 3: Storage/Broker 인터페이스 및 구현체**
  - **Output:** `StorageClient`, `BrokerClient`, GCS/PGMQ 운영 구현체, InMemory 테스트 구현체
  - **Files:** `src/infra/storage.py`, `src/infra/gcs_client.py`, `src/infra/inmemory_storage.py`, `src/infra/broker.py`, `src/infra/pgmq_client.py`, `src/infra/inmemory_broker.py`
  - **Test Files:** `tests/unit/test_storage_clients.py`, `tests/unit/test_broker.py`
  - **Commands:** `pytest tests/unit/test_storage_clients.py tests/unit/test_broker.py`
...
- [ ] **Task 4: 업로드 준비 유스케이스 구현 (`POST /api/v1/videos`)**
  - **Output:** Local File/External URL 분기, `PENDING` 저장, `storage_path` 확정, Local File Signed URL 발급, External URL `PREPROCESS_REQUEST` 발행
  - **Files:** `src/services/video_service.py`, `src/api/v1/routers/videos.py`
  - **Test Files:** `tests/unit/test_video_service_upload.py`, `tests/api/v1/test_video_create.py`
  - **Commands:** `pytest tests/unit/test_video_service_upload.py tests/api/v1/test_video_create.py`
...
- [ ] **Task 5: 업로드 완료 유스케이스 구현 (`POST /api/v1/videos/{id}/complete`)**
  - **Output:** 객체 존재/크기 검증, `PENDING -> UPLOADED`, `/complete` 멱등성, `PREPROCESS_REQUEST` 발행
  - **Files:** `src/services/video_service.py`, `src/api/v1/routers/videos.py`
  - **Test Files:** `tests/unit/test_video_service_complete.py`, `tests/api/v1/test_video_complete.py`
  - **Commands:** `pytest tests/unit/test_video_service_complete.py tests/api/v1/test_video_complete.py`
...
- [ ] **Task 6: 조회/수정/삭제/재시도/재생 URL API 구현**
  - **Output:** 목록 조회, 상세 조회, 제목/카테고리 수정, 삭제 접수, 재시도, 재생 URL 재발급 API 구현
  - **Files:** `src/services/video_service.py`, `src/api/v1/routers/videos.py`
  - **Test Files:** `tests/api/v1/test_videos.py`
  - **Commands:** `pytest tests/api/v1/test_videos.py`
...
- [ ] **Task 7: 관측성 및 공통 예외 처리 보강**
  - **Output:** 구조화 로깅, `trace_id` 전파, 핵심 메트릭, 공통 예외 매핑
  - **Files:** `src/common/logging.py`, `src/common/metrics.py`, `src/middlewares/error_handler.py`, `src/middlewares/trace.py`
  - **Test Files:** `tests/unit/test_error_handler.py`, `tests/unit/test_metrics.py`
  - **Commands:** `pytest tests/unit/test_error_handler.py tests/unit/test_metrics.py`
...
- [ ] **Task 8: 최종 통합 검증 및 릴리스 준비**
  ...
  - **Commands:** `alembic upgrade head`, `pytest`, `pytest --cov`
...
- **인프라/스키마 변경:** `alembic upgrade head`로 `video` 테이블과 인덱스를 생성한다.
- **호환성 확인:** Worker가 소비하는 `PREPROCESS_REQUEST`, `DELETE_REQUEST` 스키마와 큐 이름이 현행 spec과 일치하는지 확인한다.

### 5.2 롤백 계획 (Rollback)
- **애플리케이션 롤백:** 이전 버전의 컨테이너/아티팩트로 즉시 복귀한다.
- **데이터베이스 스키마 원복:** `alembic downgrade -1` 또는 이전 안정 버전 기준으로 복구한다. 단, 데이터 손실 영향은 사전 검토한다.
