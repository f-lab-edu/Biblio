...
* **선택 환경 설정:**
  * 단일 `/embed` 요청 허용 최대 `texts` 개수
  * 단일 텍스트 최대 길이
  * 단일 요청 허용 최대 payload size
  * 최대 동시 처리량
  * startup budget

> **Notes:** 현재 architecture 문서는 구체 모델 프레임워크(`sentence-transformers` 등)와 내부 호출 인증 방식(API key, mTLS 등)을 확정하지 않는다. 본 스펙은 특정 라이브러리나 인증 메커니즘 대신 서비스 계약과 동작 의미를 고정한다. 또한 이 스펙은 사용자 결정에 따라 Model Registry 자동 감지/자동 교체 대신, 배포 시 지정된 모델 파일을 기동 시 로드하는 단순한 운영 방식을 따른다.
...
* **모델 버전 SOT:** 현재 서빙 모델의 version string SOT는 실제 로드한 artifact path이다. path naming convention 위반으로 version 추출에 실패하면 서빙을 허용하지 않는다.
...
* **고수준 failure detail:** all-or-nothing 정책을 유지하되, 필요 시 에러 응답은 실패한 항목의 `index`와 고수준 `reason_code`(`TEXT_TOO_LONG`, `TOKENIZATION_FAILED` 등)를 포함할 수 있다. 원문 `text`, 내부 예외 메시지, 스택트레이스는 응답에 포함하지 않는다.
...
5. 최대 동시 처리량을 초과했는지 확인한다. admission control에 걸리면 `503`.
...
2. artifact path naming convention을 검증하고, 경로명에서 `model_version`을 추출한다. 추출 실패 시 기동 실패로 간주한다.
3. 모델 파일을 메모리에 로드한다.
...
* **Metrics:**
  * `embed_request_latency_ms` — `/embed` 요청 지연 시간
  * `embed_request_count` — `/embed` 요청 수
  * `embed_request_text_count` — 요청당 `texts` 개수 분포
  * `embed_error_count` — 오류 코드별 실패 수
  * `model_load_success_count` — 프로세스 시작 시 모델 로드 성공 수
  * `model_load_failure_count` — 프로세스 시작 시 모델 로드 실패 수
  * `current_embedding_model_version` — 현재 서빙 중인 모델 버전

* **Alerts:** 주요 감시 대상은 `/embed` 5xx 급증, 모델 로드 실패 반복, ready 모델 부재 상태 지속이다.
