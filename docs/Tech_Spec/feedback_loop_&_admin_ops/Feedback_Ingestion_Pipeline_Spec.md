...
### 1.2 책임 경계
- In scope:
  - Core API가 발행한 validated feedback event 소비
...
4. FIP(Vector)는 broker에서 이벤트를 비동기로 consume한다.
5. FIP는 이벤트를 Object Storage의 append-only raw log로 적재한다.
6. broker 또는 Object Storage에 일시 장애가 있으면, 즉시 영구 손실로 간주하지 않고 retry 또는 재전달 경로를 따른다.

### 1.3 기술 스택 선택
| 영역 (Area) | 선택안 (Choice) | 왜 이 선택인가 |
| --- | --- | --- |
| Ingestion runtime | Vector | `schema_version` 기반 조건부 라우팅(VRL route transform)과 멀티 sink(raw_logs/error_logs/)를 설정만으로 구현할 수 있고, at-least-once + nack 기반 retry를 브로커 source에서 기본 제공한다. Logstash 대비 경량(Rust 기반, JVM 불필요)이며, 직접 구현 대비 모니터링·버퍼·retry 인프라를 별도로 만들 필요가 없다. |
...
- Producer / consumer responsibility:
  ...
  - Consumer(FIP): 수신 이벤트를 원본 로그 형태로 Object Storage에 적재하고 trace/event 기준으로 관측 가능하게 유지
...
- Payload versioning rules:
  ...
  - 수신자(Consumer)는 자신이 지원하지 않는 `schema_version` 값을 받으면 무조건 에러로 처리하여 저장을 거부한다.
- 최소 포함 정보:
...
### 2.3 상태 및 비즈니스 규칙
- 항상 유지되어야 하는 불변조건:
  - FIP는 검증된 feedback event만 소비 대상으로 간주한다.
...
- 이 컴포넌트가 소유하는 허용 상태 전이:
  ...
  - `validated event received -> temporary retry pending` (broker/object storage 일시 실패 시)
    - retry pending 상태는 이벤트가 브로커에 nack 처리되어 visibility timeout 후 재전달 대기 중인 상태를 의미한다. Vector 크래시 시에도 브로커에 보존된다.
...
- Idempotency rule:
  ...
  - 같은 `event_id`의 재수신은 raw log에 별도 이벤트로 남길 수 있으며, `event_id`는 식별/추적 키로 사용한다.
...
- Timeout / TTL / retry constraints:
  ...
  - Object Storage / broker 일시 장애는 broker 재전달(nack/visibility timeout)로 재처리한다. 브로커 레이어의 별도 DLQ는 두지 않는다.
  - non-retryable 메시지(미지원 version, 필수 필드 누락, contract violation)는 ack 처리하여 브로커 큐에서 제거하되, Object Storage의 별도 error sink 경로(`error_logs/`)에 원본 메시지를 보존한다. 이 경로는 운영 알림 트리거 대상이며, schema 호환 후 수동 재처리 입력으로 사용할 수 있다.
  - 구체 retry 횟수/backoff/timeout 값은 FIP PLAN 문서에서 확정한다.
...
| --- | --- | --- | --- | --- |
| Broker consumer | 미지원 version 식별자 또는 필수 정보 누락 | invalid message | N | ack 후 `error_logs/` error sink에 원본 메시지 보존. 운영 알림 트리거 대상 |
| Broker consumer | Object Storage 일시 실패 | transient persistence failure | Y | 재시도 또는 재전달 대상 |
| Broker consumer | Broker 재전달로 동일 `event_id` 재수신 | duplicate delivery | Y | at-least-once의 정상 범주 |
| Broker consumer | Upstream 검증 누락으로 의미상 잘못된 데이터 수신 | contract violation | N | ack 후 `error_logs/` error sink에 원본 메시지 보존. FIP가 business validation을 재수행하지는 않지만 운영 경고 대상이다 |
...
- Key metrics / alerts worth tracking:
  ...
  - 동일한 피드백(`event_id`)이 중복으로 수신된 건수
  - 데이터 수집 지연 시간 (피드백이 큐에 들어온 시점부터 최종 저장될 때까지 걸린 시간)
  - `error_logs/` error sink에 적재된 non-retryable 메시지 건수 (알림 트리거 대상)
  - 데이터 전송 파이프라인(Vector) 내의 임시 보관량(Buffer), 재시도 현황, 정상 처리 완료(Ack) 상태를 운영자가 모니터링할 수 있어야 한다.
...
- [ ] Core API가 `SearchResponseSnapshot` 검증을 통과한 feedback event를 broker에 publish하면, FIP가 이를 소비하여 Object Storage raw log에 필요한 문맥 필드와 함께 남긴다.
...
- [ ] 미지원 version 식별자 또는 필수 필드 누락 메시지 수신 시 raw log에 정상 적재하지 않고, `error_logs/` error sink에 원본 메시지를 보존하며 운영 알림이 발생한다.
