...
- 관련 문서: `docs/Tech_Spec/upload_search_Service/Core_Api_Server_Spec.md`, `docs/Tech_Spec/upload_search_Service/Search_Service_Spec.md`
...
- 전달 의미론: at-least-once 기준. 중복 수신 가능성을 허용하고 손실 최소화를 우선한다.
...
- [ ] Core API가 `SearchResponseSnapshot` 검증을 통과한 feedback event를 broker에 publish하면, Vector 파이프라인이 이를 읽어 Object Storage raw log에 `project_id`를 포함한 필요한 문맥 필드와 함께 남긴다.
