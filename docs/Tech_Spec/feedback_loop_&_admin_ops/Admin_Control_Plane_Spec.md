...
### 1.1 한 줄 요약
- Admin Control Plane은 운영자가 project/video 처리 상태와 ML 운영 상태를 조회하고, 허용된 운영 액션만 기존 async 계약에 맞춰 안전하게 요청할 수 있게 하는 admin 전용 HTTP 표면이다.

### 1.2 책임 경계
- 범위에 포함:
  - 운영자 권한 검증을 거친 admin 전용 조회/액션 HTTP 계약
  - 소유권 제한 없이 `Project`, `Video`, `MLPipelineRun`, `ModelRelease` 상태 조회
  - 실패 video 강제 재처리 요청
...
3. 조회 요청은 shared SOT인 `Project`, `Video`, `MLPipelineRun`, `ModelRelease`를 읽어 응답한다.
...
| Storage / DB | Metadata DB (`Project`, `Video`, `MLPipelineRun`, `ModelRelease`) | 운영 조회와 동기 precondition 판단의 SOT가 이미 여기에 있다 |
...
| --- | --- | --- | --- | --- | --- |
| `/api/v1/admin/videos` | `GET` | `status?`, `failed_stage?`, `user_id?`, `project_id?`, `project_search_serving_state?`, `cursor?`, `limit?` | `200 OK`, `items[]`, `next_cursor` | admin role 필수, user ownership filter 없음 | 정렬은 `(updated_at DESC, id DESC)` |
...
응답 형태 규칙:
- `GET /api/v1/admin/videos`의 각 item은 최소 `video_id`, `project_id`, `user_id`, `status`, `failed_stage`, `project_search_serving_state`, `updated_at`를 포함한다.
...
- Payload versioning 규칙:
  - `PREPROCESS_REQUEST`, `DELETE_REQUEST`는 `docs/system-design.md` 3.13의 video-processing shared envelope를 따른다.
...
| --- | --- | --- | --- |
| Metadata DB `Project` | project serving 상태 조회 | `id`, `user_id`, `search_serving_state`, `updated_at`를 최신 기준으로 읽을 수 있어야 한다 | rollback 중 검색 제외/복귀 상태를 운영자가 추적할 수 없다 |
| Metadata DB `Video` | video 상태 조회, retry/delete precondition 확인 | `project_id`, `status`, `failed_stage`, `updated_at`를 최신 기준으로 읽고 갱신할 수 있어야 한다 | 잘못된 운영 판단 또는 잘못된 액션 허용으로 이어진다 |
...
| --- | --- | --- | --- |
| Core project lifecycle | `Project` | `id`, `user_id`, `search_serving_state`, `created_at`, `updated_at` | project serving state는 rollback exclusion 조회/필터에 사용하며, admin retry/delete 판단에는 사용하지 않는다 |
| Core video lifecycle | `Video` | `id`, `project_id`, `user_id`, `status`, `failed_stage`, `created_at`, `updated_at` | retry 가능 여부는 `status=FAILED`로만 판단한다 |
...
- 항상 유지되어야 하는 불변조건:
  ...
  - admin 조회의 project serving state는 `Project.search_serving_state`에서 읽으며, video 상태 필드로 취급하지 않는다.
...
- 거부되어야 하는 전이 / invalid condition:
  ...
  - video-level rollback exclusion field에 의존하는 동작
...
- Throughput / rate / concurrency 한계:
  ...
  - 동시에 들어온 운영 액션의 최종 적용 순서와 충돌 해결은 shared SOT(Project, Video, MLPipelineRun, ModelRelease)와 후속 처리 컴포넌트가 담당한다.
...
- [ ] admin role을 가진 운영자는 소유권 제한 없이 `Project`, `Video`, `MLPipelineRun`, `ModelRelease` 상태를 조회할 수 있다.
- [ ] admin video 목록/상세 조회는 `project_id`와 `Project.search_serving_state` 기반의 project serving state를 포함하고, 목록은 해당 필드로 필터링할 수 있다.
