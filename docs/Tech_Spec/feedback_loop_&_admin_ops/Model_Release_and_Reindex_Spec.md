...
- Upstream dependencies:
  - `MLPipelineRun`의 평가 `PASS` 결과와 후보 모델 메타데이터
...
| --- | --- | --- | --- | --- | --- |
| 평가 `PASS` 전달 | `MLPipelineRun`이 “재색인 단계로 넘길 준비 완료” 상태가 됨 | `MLPipelineRun.id`, `candidate_model_version`, 평가 성공 문맥 | candidate reindex 시작 또는 run 실패 기록 | internal only | 같은 worker 배포 단위 내부 전달이다 |
...
| `MLPipelineRun.candidate_index_name`, `MLPipelineRun.cutover_time` (shared fields) | 평가 PASS handoff 추적 | `candidate_index_name`은 candidate index 식별자, `cutover_time`은 전환 전에 candidate 반영이 끝나 있어야 하는 온라인 적재 기준 시점이다. | 나머지 run 제어는 `ML_Pipeline_Execution_Spec.md`가 소유한다 |

#### 참조 데이터 (다른 SOT를 읽는 경우)
| Source owner | Entity / table | Fields relied on | Read-only assumptions |
| --- | --- | --- | --- |
| ML Pipeline Execution | `MLPipelineRun` | `status`, `candidate_model_version`, `dataset_version`, `evaluation_id` | 평가 `PASS` handoff만 이 컴포넌트로 들어온다 |
...
- 서빙 전환 전 반영 기준:
  ...
  - 서빙 전환 직전에 이 컴포넌트가 기준 시각 `MLPipelineRun.cutover_time`을 고정한다.
...
- Idempotency rule:
  - 동일한 평가 `PASS` handoff가 재전달되어도 같은 `MLPipelineRun.id` 기준으로 candidate 상태를 중복으로 열지 않는다.
...
| --- | --- | --- | --- | --- |
| `STABLE` | `CANDIDATE_REINDEXING` | 평가 `PASS` handoff 수신 | candidate 전환 중인 다른 release가 없어야 함 | `candidate_model_version/candidate_index_name` 설정, candidate index 생성 시작, online ingest dual-write 개시 |
...
| --- | --- | --- | --- | --- |
| 평가 `PASS` handoff | `MLPipelineRun`이 release/reindex 진입 가능 상태가 아님 | invalid handoff | N | current serving과 `ModelRelease`는 유지한다 |
...
- Trace / correlation propagation rule:
  - 평가 `PASS` handoff, `ROLLBACK_REQUEST` control message, `ModelRelease` update, candidate/rollback model load, 관련 reindex 작업은 동일 `trace_id`로 연결해야 한다.
...
- [ ] 평가 `PASS` handoff가 들어오면 `ModelRelease`가 `CANDIDATE_REINDEXING`으로 전환되고 `candidate_model_version/candidate_index_name`이 채워지며, end-user serving 조합은 아직 바뀌지 않는다.
...
- [ ] 동일한 평가 `PASS` handoff 또는 rollback 요청이 중복 수신되어도 duplicate cutover 또는 duplicate restore가 발생하지 않는다.
