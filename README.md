...
이 프로젝트에서는 AI를 단순 코드 생성기가 아니라, 문제를 나누고 선택지를 검토하며 구현 결과를 다시 검증하는 개발 파트너로 사용했습니다. 다만 AI의 완료 보고를 그대로 믿지 않고 최종 판단은 코드·테스트·실행 결과를 기준으로 내렸습니다.

| 단계 | AI 활용 | 검증 방법 |
| --- | --- | --- |
| 문제 정의 | 요구사항을 완료 조건과 실패 조건으로 분해 | [PRD](./docs/PRD.md), 기능별 완료 기준 |
| 설계 | 여러 구현안과 장애 경로, 비용·운영 trade-off 비교 | [Tech Spec](./docs/Tech_Spec), [ADR](./docs/ADR) |
| 구현 | 저장소 문맥과 변경 범위를 제공해 작은 작업 단위로 위임 | 단위·통합·API 테스트 |
| 리뷰 | 구현과 설계의 불일치, 예외 처리와 운영 위험 탐색 | [리뷰 기록](./docs/reviews), 정적 분석 |
| 운영 검증 | 로그와 실제 증상을 바탕으로 원인 후보와 검증 순서 구성 | [백엔드 E2E](./scripts/e2e), [GCP 배포 Runbook](./docs/runbooks/gcp-performance-deployment.md) |

AI와 협업한 사실보다 다음 과정을 재현 가능하게 남기는 데 집중했습니다.

- 구현 전에 완료 조건과 책임 경계를 문서로 고정했습니다.
- 선택하지 않은 대안과 이유를 ADR에 기록했습니다.
- 실패한 테스트를 수정한 뒤 같은 테스트를 다시 통과시켰습니다.
- 로컬 테스트로 끝내지 않고 실제 GCP 환경에서 서비스 간 연결과 복구 흐름을 검증했습니다.
